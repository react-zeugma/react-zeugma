...
export function CopilotWidget() {
  ...
  useEffect(() => {
    if (messages.length === 0 && layout) {
      ...
      setMessages(INITIAL_MESSAGES(summary))
    }
  }, [layout])
  ...
  return (
    <div className="flex flex-col h-full w-full bg-[#1e1e1e] overflow-hidden text-xs text-zinc-300">
      ...
      {/* Messages area */}
      <div className="flex-1 overflow-y-auto p-4 space-y-4 scrollbar-thin select-text min-h-0">
        {messages.map((msg) => {
        ...
        {isTyping && (
          <div className="flex gap-3 items-start animate-pulse">
            ...
            <div className="bg-[#252526] border border-[#2d2d30] rounded-lg px-3 py-2.5 flex items-center gap-1.5">
              <span className="w-1.5 h-1.5 bg-zinc-550 rounded-full animate-bounce [animation-delay:-0.3s]" />
              <span className="w-1.5 h-1.5 bg-zinc-550 rounded-full animate-bounce [animation-delay:-0.15s]" />
              <span className="w-1.5 h-1.5 bg-zinc-550 rounded-full animate-bounce" />
            </div>
          </div>
        )}
